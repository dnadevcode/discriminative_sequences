...
newscorefun = @(x,l1,l2,w) norminv(beta_ev_cdf(x,nuF*w,1,max(0.2*2*(max(l1,l2)-w),0.001*2*(l1-w+1)*(l2-w+1)),0));

newscorefun2 = @(x,l1,l2,w) norminv(beta_ev_cdf(x,nuF*l1,1,2*l2,0));
...
for i =1:length(rM)
    ...
    for j=1:length(rM{i})
        ...
        if rM{i}{j}{k}.maxcoef(1) > minCC % fix stretch factor as the smallest
            rM{i}{j}{k}.stoufferScore = fun(rM{i}{j}{k}.maxcoef(1),rM{1}{j}{k}.lengthMatch/rM{1}{j}{k}.bestBarStretch*0.9, theoryStruct(i).length,rM{i}{j}{k}.lengthMatch);
        else
